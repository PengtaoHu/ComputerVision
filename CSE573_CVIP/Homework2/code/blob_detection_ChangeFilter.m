function blob_detection_ChangeFilter()
...
i=0:14;
ks=k.^i;
filter0=fspecial('log',[5 5],0.8);
s0=sum(sum(abs(filter0)));
r=zeros(level_num);

for i=1:level_num
    mask_size=round(1.5/ks(i))*2+1;
    filter=fspecial('log',[mask_size mask_size],0.8/ks(i));
...
saveas(gcf,strrep(im_Path,'.jpg','_output1.jpg'));